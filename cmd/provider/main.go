...
import (
	...
	apisCluster "github.com/arkilasystems/provider-vcd/apis/cluster"
	apisNamespaced "github.com/arkilasystems/provider-vcd/apis/namespaced"
	"github.com/arkilasystems/provider-vcd/config"
	"github.com/arkilasystems/provider-vcd/internal/clients"
	controllerCluster "github.com/arkilasystems/provider-vcd/internal/controller/cluster"
	controllerNamespaced "github.com/arkilasystems/provider-vcd/internal/controller/namespaced"
	"github.com/arkilasystems/provider-vcd/internal/features"
)

func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Vcd").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-vcd"))
	if *debug {
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped Vcd controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced Vcd controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped Vcd controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced Vcd controllers")
	}
